...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	 ...
	 */
	 ...
	 if (gotFreeSlot == false) {
		 printf("No more free slot in soundBites[]\n");
		 return;
		 // exit(EXIT_FAILURE);
	 }
...
void AudioMixer_volumeUp(void) {
	int newVol = volume + AUDIOMIXER_VOLUME_PER_CHANGE;
	if (AUDIOMIXER_MIN_VOLUME <= newVol && newVol <= AUDIOMIXER_MAX_VOLUME) {
...
static void fillPlaybackBuffer(short *playbackBuffer, int size) {
	// printf("fillPlaybackBuffer is called\n");
	/*
	 * REVISIT: Implement this
	 ...
	 */
	 ...
	 // *    may be used by any other thread, must synchronize this.
	 pthread_mutex_lock(&audioMutex);
	 for (int i = 0; i < MAX_SOUND_BITES; i++) { // 0~29
		 wavedata_t* pSound_i = soundBites[i].pSound;
		 if (pSound_i != NULL) {
			 int numSamples_pSound_i = pSound_i->numSamples;
			 short* pData_pSound_i = pSound_i->pData;

			 int location_i = soundBites[i].location;
			 int j = 0;
			 for (j = 0; j < size; j++) {
				 if ((location_i + j) < numSamples_pSound_i) { // if not reach end of sound;
					 if (playbackBuffer[j] > SHRT_MAX - pData_pSound_i[j + location_i]){ // handle overflow(only two positives)
						 playbackBuffer[j] = SHRT_MAX;
					 } else if (playbackBuffer[j] < SHRT_MIN - pData_pSound_i[j + location_i]) { // handle underflow (only two negatives)
						 playbackBuffer[j] = SHRT_MIN;
					 } else {
					 	playbackBuffer[j] += pData_pSound_i[j+location_i];
					}
				} else { // end of sound, free slot
					 soundBites[i].pSound = NULL;
					 soundBites[i].location = 0;
					 break;
				 }
...
void sleepForPiontOneSecond() {
	// sellp for 1.5 seconds
	long seconds = 0;
